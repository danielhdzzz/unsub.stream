...
async function handleLastfmSession(request, env) {
  try {
    ...
    if (data.error) {
      return jsonResponse({ error: data.message || "Auth failed" }, 400, request);
    }
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/search") return handleSearch(url, env, request);
    if (url.pathname === "/lastfm/auth") return handleLastfmAuth(url, env, request);
    if (url.pathname === "/lastfm/session" && request.method === "POST") return handleLastfmSession(request, env);
    if (url.pathname === "/lastfm/nowplaying" && request.method === "POST") return handleLastfmNowPlaying(request, env);
    if (url.pathname === "/lastfm/scrobble" && request.method === "POST") return handleLastfmScrobble(request, env);
